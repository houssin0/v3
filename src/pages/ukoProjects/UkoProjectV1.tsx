...
import { Button, Card, Grid, Tab, Theme, useMediaQuery } from "@mui/material";
import { Box, styled } from "@mui/system";
import FlexBox from "components/FlexBox";
import MoreOptions from "components/MoreOptions";
import SearchInput from "components/SearchInput";
import { H2 } from "components/Typography";
import ProjectCard from "components/ukoProjects/ProjectCard";
import TabLabel from "components/ukoProjects/TabLabel";
import useTitle from "hooks/useTitle";
import { FC, MouseEvent, SyntheticEvent, useState } from "react";
import CreateProject from "../../components/ukoProjects/CreateProject";
...
const StyledTabList = styled(TabList)(({ theme }) => ({
  "& .MuiTabs-flexContainer": {
    ...
    "& .Mui-selected": {
      "& h2, & h5": { color: theme.palette.primary.main },
    },
  },
}));

const UkoProjectV1: FC = () => {
  // change navbar title
  useTitle("Uko Project V1");

  const [value, setValue] = useState("0");
  const [openModal, setOpenModal] = useState(false);
  const [projectMoreEl, setProjectMoreEl] = useState<null | HTMLElement>(null);
      ...
      <TabContext value={value}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={5}>
            <Card sx={{ padding: ".1rem 2rem", height: "100%" }}>
              <FlexBox
                height="100%"
                alignItems="center"
                justifyContent="space-between"
