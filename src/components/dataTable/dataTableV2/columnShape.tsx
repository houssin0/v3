...
const CommonCell = ({ title, body }: any) => (
...
export function SelectColumnFilter({ column }: any) {
  ...
  return (
    ...
    >
      <MenuItem value="" sx={{ fontSize: 12, fontWeight: 500 }}>
        All
      </MenuItem>
      {options.map((option, i) => (
        <MenuItem key={i} value={option} sx={{ fontSize: 12, fontWeight: 500 }}>
          {option}
        </MenuItem>
      ))}
    </Select>
...
export function DateColumnFilter({ column }: any) {
